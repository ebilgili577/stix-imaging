...
def stx_plot_vis_fit(vis, vis_pred, sigamp):
    ...
    phase_vis = np.arctan2(im_vis, re_vis) / np.pi * 180.0  # In degrees

    sigphase = (
        np.divide(sigamp, amp_vis, out=np.zeros_like(sigamp), where=amp_vis != 0)
        / np.pi
        * 180.0
    )
    ...
    phase_vis_pred = np.arctan2(im_vis_pred, re_vis_pred) / np.pi * 180.0  # In degrees
    ...
    fig, axs = plt.subplots(
        2, 2, sharex=True, figsize=(15, 7), gridspec_kw={"height_ratios": [2, 1]}
    )
    ...
    axs[1, 0].plot(
        xx,
        (phase_vis[idx_vis] - phase_vis_pred[idx_vis]) / sigphase[idx_vis],
        "+",
        color="k",
        markersize=markersize,
    )
    ...
    axs[1, 1].plot(
        xx,
        (amp_vis[idx_vis] - amp_vis_pred[idx_vis]) / sigamp[idx_vis],
        "+",
        color="k",
        markersize=markersize,
    )
