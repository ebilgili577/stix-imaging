...
def get_hpc_coords(flare_loc: SkyCoord, t_center, observer):
    ...
    flare_hpc = flare_loc.transform_to(Helioprojective(ovserver=observer, obstime=t_center))
    return flare_hpc
...
def get_sun_radius(observer):
    ...
    """
    rsun = (
        (sun_const.radius / (observer.spherical.distance - sun_const.radius)).decompose()
        * u.radian
    ).to_value(u.arcsec)
    return float(rsun)
